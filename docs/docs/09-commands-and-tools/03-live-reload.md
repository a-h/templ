...
If the `*_templ.go` files change, #3 and #4 must be ran.

:::note
Templ only watches the .go files that are generated by templ. The recommendation is to use `templ --watch` in conjuction with other watch tools like [Air](https://github.com/air-verse/air) or [wgo](https://github.com/bokwoon95/wgo). See [joerdav/shopping-list](https://github.com/joerdav/shopping-list) for an example.
:::
